class PlayFairCipher:
    ...
    def create_matrix(self, key):
        ...
        # Ensure the matrix has 25 letters
            if len(matrix)  == 25:
                break 
        playfair_matrix = [matrix[i:i + 5] for i in range(0, len(matrix), 5)]
        return playfair_matrix
    def find_letter_coordinates(self, letter, matrix):
    ...
    def playfair_encrypt(self, plain_text, matrix):
        ...
        for i in range(0, len(plain_text), 2):
            ...
            if len(pair) == 1:
                pair += 'X'
            row1, col1 = self.find_letter_coordinates(matrix,  pair[0] )
            row2, col2 = self.find_letter_coordinates( matrix , pair[1])
            if row1 == row2:
    ...
    def playfair_decrypt(self, cipher_text, matrix):
        ...
        banro = ""
        #loại bỏ kí tự 'X' ở cuối nếu nó được thêm vào
